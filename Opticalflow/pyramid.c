     ...
     int nx, ny;
{
  ...
  for(i=1;i<py;i++){
    for(j=1;j<px;j++){
      index = (i*px) + j;
      if ((i%2 == 0) && (j%2 == 0)&&i<py-1&&j<px-1) {
	proj[index] = (proj[(i-1)*px + (j-1)]
			  + proj[(i+1)*px + (j-1)] 
			  + proj[(i-1)*px + (j+1)] 
			  + proj[(i+1)*px + (j+1)])/4.0;
      } 
      
      else if (i%2 == 0&&i<py-1) {
	proj[index] = (proj[(i-1)*px + j] + proj[(i+1)*px + j])/2.0;
      }
      
      else if (j%2 == 0) {
	proj[index] = (proj[index-1] + proj[index+1])/2.0;
      }
