...
class CImageL : public CImageAttributes
{
public:
    CImageL(void);               // default constructor
    CImageL(CShape s, const type_info& ti, int bandSize);
...
inline void* CImageL::PixelAddress(int x, int y, int band)
{
...
CImageL(CShape(), typeid(T), sizeof(T)) {}

template <class T>
inline CImageOf<T>::CImageOf(CShape s) :
CImageL(s, typeid(T), sizeof(T)) {}

template <class T>
inline CImageOf<T>::CImageOf(int width, int height, int nBands) :
CImageL(CShape(width, height, nBands), typeid(T), sizeof(T)) {}

template <class T>
inline void CImageOf<T>::ReAllocate(CShape s, bool evenIfSameShape)
{
    CImageL::ReAllocate(s, typeid(T), sizeof(T), evenIfSameShape);
}
...
{
    CImageL::ReAllocate(s, typeid(T), sizeof(T), memory, deleteWhenDone, rowSize);
}
