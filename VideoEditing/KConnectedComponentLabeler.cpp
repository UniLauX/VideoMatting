...
void KConnectedComponentLabeler::InitConfig(int nAreaThreshold)
{
...
//		saveBMP("ccl_.bmp",m_width,m_height,m_MaskArray,'g');
		...
		for(i=0; i<label; i++)
		{
			...
			KBox newComponent;
			newComponent.ID = i+1;
			newComponent.bottomRight = CPoint(INT_MAX, INT_MAX);
			newComponent.topLeft     = CPoint(INT_MIN, INT_MIN);
...
void KConnectedComponentLabeler::CheckConnectivity(unsigned char* l, int minarea)
{
	Clear();
	if(m_MaskArray == NULL) return;
...
void KConnectedComponentLabeler::FillHoles(int minarea)
{
		Clear();
	if(m_MaskArray == NULL) return;
...
/*				if( index/m_width == 0 ) 
	...
	if( regionNumber > 0 ) 
	{
		...
		for (int i=0; i<size; ++i)
		{
			if (regionArea[m_MaskArray[i]]<minarea)
			{
				m_MaskArray[i] = 1;
			}
		}
...
void KConnectedComponentLabeler::OnCalculateHex(int *array, int a)
{
	for(int i=0; i<4; i++)
	{
		array[i] = a%256;	
...
KConnectedComponentLabeler::KBox::KBox()
{
	ID = 0;
	bottomRight = 0;
	topLeft = 0;
...
void KConnectedComponentLabeler::KLinkedList::printTable()
{
	KNode* ptr1 = header;
	KNode* ptr2 = header;
...
void KConnectedComponentLabeler::SetMask(BYTE *mask, int width, int height)
{
	if( m_MaskArray != NULL )
		delete []m_MaskArray;
...
void KConnectedComponentLabeler::Binarize()
{
	if( m_MaskArray == NULL )
		return;
...
void KConnectedComponentLabeler::Clear()
{
	m_Components.clear();
	m_ObjectNumber = 0;
}

void KConnectedComponentLabeler::Merge()
{
	...
	BYTE *equalTable = new BYTE[sz*sz];
