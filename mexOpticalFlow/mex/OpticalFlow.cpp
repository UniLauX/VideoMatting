...
void OpticalFlow::getDxs(DImage &imdx, DImage &imdy, DImage &imdt, const DImage &im1, const DImage &im2)
{
	...
	im1.imfilter_hv(Im1,gfilter,2,gfilter,2);   //have accelerated 
	im2.imfilter_hv(Im2,gfilter,2,gfilter,2);   //have accelerated
	...
	Im2.dx(imdx,true);
	Im2.dy(imdy,true);
	imdt.Subtract(Im2,Im1);          //have accelerated
	imdx.setDerivative();            //not necessary
	imdy.setDerivative();
	imdt.setDerivative();
}
...
#pragma omp parallel for if(THREAD_SWITCH)
			...
			ImDxy.Multiply(Psi_1st,imdx,imdy);      //have accelerated
			...
			if(nChannels>1)
			{
				ImDxy.collapse(imdxy);            // have accelerated
			...
			// filtering
			imdx2.smoothing(A11,3);  // can't accelerate
			imdxy.smoothing(A12,3);
			imdy2.smoothing(A22,3);

			// add epsilon to A11 and A22
			A11.Add(alpha*0.1);   //have accelerated
			A22.Add(alpha*0.1);
																	 ...
																	 int nOuterFPIterations, int nInnerFPIterations, int nCGIterations)
{
	...
	GPyramid1.ConstructPyramid(Im1,ratio,minWidth);    //can't accelerate
	...
	for(int k=GPyramid1.nlevels()-1;k>=0;k--)
	{
		...
		im2feature(Image1,GPyramid1.Image(k));      //have accelerated      
		...
		else
		{
			...
			vx.imresize(width,height);    //can't accelerate
			vx.Multiplywith(1/ratio);     //have accelerated
			vy.imresize(width,height);
			vy.Multiplywith(1/ratio);
			//warpFL(warpI2,GPyramid1.Image(k),GPyramid2.Image(k),vx,vy);
			warpFL(WarpImage2,Image1,Image2,vx,vy);    //have accelerated
		}
